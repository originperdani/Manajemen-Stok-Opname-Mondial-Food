    ...
    <div class="detail-product-card" style="padding: 1.5rem;">
            ...
            @if($transaksi->pembayaran)
                <div class="receipt-actions">
                    <a href="{{ route('penjualan.transaksi.struk', $transaksi) }}" target="_blank" rel="noopener noreferrer" class="btn btn-primary">
                        <i class="fas fa-receipt"></i> Lihat Struk
                    </a>
                    <a href="{{ route('penjualan.transaksi.struk.download', $transaksi) }}" class="btn btn-secondary">
            ...
            @php
                $isPickup = $transaksi->pengiriman && in_array($transaksi->pengiriman->metode_kirim, ['ambil_sendiri']);
                $statusOptions = ['pending', 'diproses', 'dikirim', 'selesai', 'dibatalkan'];
                $statusOrder = ['pending' => 0, 'diproses' => 1, 'dikirim' => 2, 'selesai' => 3];
                $terminalStatuses = ['selesai', 'dibatalkan'];
                $isTerminalStatus = in_array($transaksi->status, $terminalStatuses, true);
                $currentStatusOrder = $statusOrder[$transaksi->status] ?? null;
                $isPaymentNotSuccessful = !$transaksi->pembayaran || $transaksi->pembayaran->status !== 'berhasil';
                $onlyAllowCancel = $isPaymentNotSuccessful;
            @endphp
            <form method="POST" action="{{ route('penjualan.transaksi.status', $transaksi) }}" class="action-form">
                @csrf
                @method('PUT')
                <select name="status" class="form-control" id="statusSelect" {{ $isTerminalStatus || ($onlyAllowCancel && !in_array($transaksi->status, ['belum_bayar', 'dibatalkan'])) ? 'disabled' : '' }}>
                    @foreach($statusOptions as $s)
                        @php
                            $label = str_replace('_', ' ', $s);
                            if ($isPickup && $s === 'dikirim') {
                                $label = 'siap diambil';
                            }

                            $optionOrder = $statusOrder[$s] ?? null;
                            $isPreviousStatus = $currentStatusOrder !== null
                                && $optionOrder !== null
                                && $optionOrder < $currentStatusOrder;
                            $isDisabled = $isTerminalStatus || $isPreviousStatus || ($onlyAllowCancel && $s !== 'dibatalkan');
                        @endphp
                        <option value="{{ $s }}" {{ $transaksi->status==$s?'selected':'' }} {{ $isDisabled ? 'disabled' : '' }}>
                            {{ ucfirst($label) }}
                        </option>
                    @endforeach
                </select>
                <button class="btn" {{ $isTerminalStatus || ($onlyAllowCancel && !in_array($transaksi->status, ['belum_bayar', 'dibatalkan'])) ? 'disabled' : '' }}>Update</button>
            </form>
            @if($isTerminalStatus)
                <p class="action-note">Status pesanan sudah final dan tidak bisa diubah lagi.</p>
            @endif
            @if($onlyAllowCancel && !$isTerminalStatus)
                <p class="action-note" style="color: #dc2626;">Pesanan belum dibayar, hanya bisa dibatalkan.</p>
            @endif
            @if($transaksi->tipe == 'online' && !$transaksi->pengiriman && !$isPaymentNotSuccessful)
            <form method="POST" action="{{ route('penjualan.transaksi.kirim', $transaksi) }}" class="action-form">
                @csrf
                <select name="metode_kirim" class="form-control">
                    <option value="kurir_toko">Kurir Toko</option>
                    <option value="kurir_ojol">Kurir Toko / Ojek Online</option>
                    <option value="grabfood">GrabFood</option>
                    <option value="gofood">GoFood</option>
                    <option value="ambil_sendiri">Ambil Sendiri</option>
                </select>
                <button class="btn" style="background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%);">Kirim</button>
            </form>
            @endif
        </div>
</div>
